// UNIQUE TO DEVICE DATA 
      
// The below codes are for the demonstration device only.
// Unique codes will be assigned to your company when you purchase devices. 

//#define ValidateCode1 0X488B // Codes used as part of active algorithm to
//#define ValidateCode2 0XFEE2 // check for installation of proper device
//#define ValidateCode3 0XEF90
//#define ClientIDCode1 0XD882
//#define ClientIDCode2 0X5E01
//#define ReadCode1     0X1772 // Codes used to authorize read operations
//#define ReadCode2     0XC4E6
//#define ReadCode3     0XBCF8
//#define WriteCode1    0X30A0 // Codes used to authorize write operations
//#define WriteCode2    0X3C89
//#define WriteCode3    0X0A2B


/* The following codes are unique to Radiant and should  be kept confidential. */
#define ValidateCode1 0x2FD6
#define ValidateCode2 0x8F4
#define ValidateCode3 0xB702
#define ClientIDCode1 0xCE74
#define ClientIDCode2 0x366
#define ReadCode1     0x4A15
#define ReadCode2     0xD210
#define ReadCode3     0x2EA0
#define WriteCode1    0x2656
#define WriteCode2    0x61EE
#define WriteCode3    0x9873
#define RemoteUpdateEncryptKey    0x7A724257

//#define UserPin       "45EXabQF"
