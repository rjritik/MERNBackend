...
void CheckLicense() {
  ...
  if ((ReturnValue1 == ClientIDCode1) && (ReturnValue2 == ClientIDCode2))
  {
    ...
    if(ReturnValue1 == 2) {
        ...
        KTASK(GETVARWORD, 2, 0, 0);
        printf("%u",ReturnValue1);
    }
  }
}
...
{
  if(strcmp(argv[1], "kV8s$^HcQ-QV+nHr^eUuJr8puB7Lg6zz") == 0) {
    if(strcmp(argv[2], "check-key") == 0) {
      CheckKey();
    }
    if(strcmp(argv[2], "check-license") == 0) {
      CheckLicense();
    }
  }
	return 0;
